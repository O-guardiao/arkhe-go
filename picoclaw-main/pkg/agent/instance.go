...
func NewAgentInstance(
	...
	primaryAndFallbacks := append([]string{model}, fallbacks...)
	populateCandidateProvidersFromNames(cfg, workspace, primaryAndFallbacks, candidateProviders)
	if len(candidates) > 0 {
		if primaryProvider, ok := candidateProviders[providers.ModelKey(candidates[0].Provider, candidates[0].Model)]; ok {
			...
			if err != nil {
				...
				if err != nil {
					...
					if len(lightCandidates) > 0 {
						if boundLightProvider, ok := candidateProviders[providers.ModelKey(
							lightCandidates[0].Provider,
							lightCandidates[0].Model,
						)]; ok {
							lightProvider = boundLightProvider
						}
					}
				}
			}
...
func (a *AgentInstance) closeProviders() {
	seen := make(map[string]struct{})
	closeProvider := func(provider providers.LLMProvider) {
		if provider == nil {
			return
		}
		...
		identity := providerIdentity(stateful)
	...
	closeProvider(a.LightProvider)
	for _, provider := range a.CandidateProviders {
		closeProvider(provider)
	}
}

func providerIdentity(provider any) string {
	if provider == nil {
		return ""
	}
	...
	if v.IsValid() {
		...
		case reflect.Pointer, reflect.Map, reflect.Slice, reflect.Func, reflect.Chan, reflect.UnsafePointer:
			return fmt.Sprintf("%T:%x", provider, v.Pointer())
		}
	}
