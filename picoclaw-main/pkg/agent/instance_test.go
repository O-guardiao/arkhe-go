...
func (m *statefulMockProvider) GetDefaultModel() string {
	return "stateful-mock"
}

func (m *statefulMockProvider) Close() {
	m.closed.Add(1)
}
...
func TestAgentInstanceClose_ClosesUniqueStatefulProviders(t *testing.T) {
	primary := &statefulMockProvider{}
	fallback := &statefulMockProvider{}

	agent := &AgentInstance{
		Provider:      primary,
		LightProvider: primary,
	...
	if primary.closed.Load() != 1 {
		t.Fatalf("primary close count = %d, want 1", primary.closed.Load())
	}
	if fallback.closed.Load() != 1 {
		t.Fatalf("fallback close count = %d, want 1", fallback.closed.Load())
	}
}
