...
func (al *AgentLoop) ReloadProviderAndConfig(
	...
	al.bindRuntimeProvidersForRegistry(registry)
	...
	// Close old stateful providers after releasing the lock.
	// This prevents blocking readers while cleanup runs.
	if oldRegistry != nil {
		select {
		case <-time.After(100 * time.Millisecond):
			closeRegistryProviders(oldRegistry)
		case <-ctx.Done():
...
func (al *AgentLoop) bindRuntimeProviders() {
	al.bindRuntimeProvidersForRegistry(al.registry)
}

func (al *AgentLoop) bindRuntimeProvidersForRegistry(registry *AgentRegistry) {
	...
	for _, agentID := range registry.ListAgentIDs() {
		...
		agentRef := agent
		binding := providers.AgentRuntimeBinding{
			ExecuteTool: func(ctx context.Context, name string, args map[string]any, meta providers.ToolCallContext) providers.ToolExecutionResult {
				return al.executeToolForRLM(ctx, agentRef, name, args, meta)
			},
			Workspace: agentRef.Workspace,
		}

		bind := func(provider providers.LLMProvider) {
			if provider == nil {
				return
			}
			if runtimeBindable, ok := provider.(providers.RuntimeBindable); ok {
				runtimeBindable.BindRuntime(binding)
			}
		}

		bind(agent.Provider)
		bind(agent.LightProvider)
		for _, candidateProvider := range agent.CandidateProviders {
			bind(candidateProvider)
		}
	}
}

func (al *AgentLoop) executeToolForRLM(
	ctx context.Context,
	agent *AgentInstance,
	toolName string,
	toolArgs map[string]any,
	meta providers.ToolCallContext,
) providers.ToolExecutionResult {
	if agent == nil || agent.Tools == nil {
		return providers.ToolExecutionResult{
			Content: fmt.Sprintf("Error: tool runtime is not available for %q", toolName),
			IsError: true,
		}
	}
	if toolArgs == nil {
	...
	toolResult := agent.Tools.ExecuteWithContext(execCtx, toolName, toolArgs, meta.Channel, meta.ChatID, nil)
	if toolResult == nil {
		toolResult = tools.ErrorResult("tool execution returned nil result")
	}

	shouldSendForUser := !toolResult.Silent && toolResult.ForUser != "" &&
		meta.Channel != "" && meta.ChatID != ""
	if shouldSendForUser && al.bus != nil {
	...
	if len(toolResult.Media) > 0 && toolResult.ResponseHandled {
		...
		for _, ref := range toolResult.Media {
			...
			if al.mediaStore != nil {
				if _, mediaMeta, err := al.mediaStore.ResolveWithMeta(ref); err == nil {
					part.Filename = mediaMeta.Filename
					part.ContentType = mediaMeta.ContentType
					part.Type = inferMediaType(mediaMeta.Filename, mediaMeta.ContentType)
				}
			}
		...
		if al.channelManager != nil && meta.Channel != "" && !constants.IsInternalChannel(meta.Channel) {
			if err := al.channelManager.SendMedia(ctx, outboundMedia); err != nil {
				toolResult = tools.ErrorResult(fmt.Sprintf("failed to deliver attachment: %v", err)).WithError(err)
			}
		} else if al.bus != nil {
			_ = al.bus.PublishOutboundMedia(ctx, outboundMedia)
	...
	if al.cfg != nil && al.cfg.Tools.IsFilterSensitiveDataEnabled() {
		contentForLLM = al.cfg.FilterSensitiveData(contentForLLM)
	}
	if strings.TrimSpace(contentForLLM) == "" {
		if toolResult.IsError {
			contentForLLM = fmt.Sprintf("Error: tool %q failed without output", toolName)
		} else {
			contentForLLM = fmt.Sprintf("Tool %q completed", toolName)
		}
	}
...
turnLoop:
	for ts.currentIteration() < ts.agent.MaxIterations || len(pendingMessages) > 0 || func() bool {
		...
		llmOpts := map[string]any{
			...
			"rlm_agent_id":            ts.agent.ID,
			"rlm_session_key":         ts.sessionKey,
			"rlm_channel":             ts.channel,
			"rlm_chat_id":             ts.chatID,
			"rlm_message_id":          ts.opts.MessageID,
			"rlm_reply_to_message_id": ts.opts.ReplyToMessageID,
		}
		if useNativeSearch {
...
func closeRegistryProviders(registry *AgentRegistry) {
	if registry == nil {
		return
	}
	for _, agentID := range registry.ListAgentIDs() {
