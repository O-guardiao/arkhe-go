...
const (
	googleAntigravityClientIDEnv     = "PICOCLAW_ANTIGRAVITY_CLIENT_ID"
	googleAntigravityClientSecretEnv = "PICOCLAW_ANTIGRAVITY_CLIENT_SECRET"
)
...
// Client credentials must be supplied through environment variables so they are not committed to git history.
func GoogleAntigravityOAuthConfig() OAuthProviderConfig {
	return OAuthProviderConfig{
		Issuer:       "https://accounts.google.com/o/oauth2/v2",
		TokenURL:     "https://oauth2.googleapis.com/token",
		ClientID:     strings.TrimSpace(os.Getenv(googleAntigravityClientIDEnv)),
		ClientSecret: strings.TrimSpace(os.Getenv(googleAntigravityClientSecretEnv)),
		Scopes:       "https://www.googleapis.com/auth/cloud-platform https://www.googleapis.com/auth/userinfo.email https://www.googleapis.com/auth/userinfo.profile https://www.googleapis.com/auth/cclog https://www.googleapis.com/auth/experimentsandconfigs",
		Port:         51121,
	}
}

func ValidateGoogleAntigravityOAuthConfig(cfg OAuthProviderConfig) error {
	missing := make([]string, 0, 2)
	if strings.TrimSpace(cfg.ClientID) == "" {
		missing = append(missing, googleAntigravityClientIDEnv)
	}
	if strings.TrimSpace(cfg.ClientSecret) == "" {
		missing = append(missing, googleAntigravityClientSecretEnv)
	}
	if len(missing) == 0 {
		return nil
	}
	return fmt.Errorf(
		"google antigravity OAuth requires %s to be set",
