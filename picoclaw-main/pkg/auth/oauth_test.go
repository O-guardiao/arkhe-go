...
func TestGoogleAntigravityOAuthConfigFromEnv(t *testing.T) {
	t.Setenv(googleAntigravityClientIDEnv, "test-google-client-id")
	t.Setenv(googleAntigravityClientSecretEnv, "test-google-client-secret")

	cfg := GoogleAntigravityOAuthConfig()

	if cfg.ClientID != "test-google-client-id" {
		t.Errorf("ClientID = %q, want %q", cfg.ClientID, "test-google-client-id")
	}
	if cfg.ClientSecret != "test-google-client-secret" {
		t.Errorf("ClientSecret = %q, want %q", cfg.ClientSecret, "test-google-client-secret")
	}
	if err := ValidateGoogleAntigravityOAuthConfig(cfg); err != nil {
		t.Fatalf("ValidateGoogleAntigravityOAuthConfig() error = %v, want nil", err)
	}
}

func TestValidateGoogleAntigravityOAuthConfigMissingEnv(t *testing.T) {
	err := ValidateGoogleAntigravityOAuthConfig(OAuthProviderConfig{})
	if err == nil {
		t.Fatal("expected validation error when antigravity credentials are missing")
	}
	msg := err.Error()
	if !strings.Contains(msg, googleAntigravityClientIDEnv) {
		t.Fatalf("validation error %q does not mention %s", msg, googleAntigravityClientIDEnv)
	}
	if !strings.Contains(msg, googleAntigravityClientSecretEnv) {
		t.Fatalf("validation error %q does not mention %s", msg, googleAntigravityClientSecretEnv)
	}
}
