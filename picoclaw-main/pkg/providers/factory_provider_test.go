...
func TestCreateProviderFromConfig_RLM(t *testing.T) {
	cfg := &config.ModelConfig{
		ModelName: "local-rlm",
		Model:     "rlm/lmstudio/qwen2.5-coder",
	...
	if modelID != "lmstudio/qwen2.5-coder" {
		t.Fatalf("modelID = %q, want %q", modelID, "lmstudio/qwen2.5-coder")
	}
	if _, ok := provider.(*RLMProvider); !ok {
		t.Fatalf("expected *RLMProvider, got %T", provider)
	}
}

func TestCreateProviderFromConfig_RLMRejectsRemoteAPIBase(t *testing.T) {
	cfg := &config.ModelConfig{
		ModelName: "bad-rlm",
		Model:     "rlm/openai/local-model",
		APIBase:   "https://api.openai.com/v1",
	}

	_, _, err := CreateProviderFromConfig(cfg)
	if err == nil {
		t.Fatal("CreateProviderFromConfig() expected error for remote rlm api_base")
	}
	if !strings.Contains(err.Error(), "local api_base") {
