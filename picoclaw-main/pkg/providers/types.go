...
// ToolCallContext carries request-scoped routing metadata so providers with an
// internal tool runtime can execute PicoClaw tools faithfully.
...
// ToolExecutionResult is the normalized result returned by agent-scoped tool
// executors exposed to providers such as the local RLM bridge.
type ToolExecutionResult struct {
	Content string
	IsError bool
}

// AgentRuntimeBinding exposes local tool execution back into providers that
// need to run PicoClaw tools internally while remaining fully local.
...
// RuntimeBindable is implemented by providers that need agent-scoped runtime
// bindings after tool registries and local execution surfaces are initialized.
type RuntimeBindable interface {
	BindRuntime(binding AgentRuntimeBinding)
}
